...
## 토크나이징된 뉴스 2gb(1024) 데이터 불러오기(해당경로에는 2gb를 미리 토크나이징한 데이터가 있당.)
dataset = load_dataset("leeseeun/tokenzied_news_2gb_data")
...
max_length = 2048 # get_max_length(model) # Llama2 4096
...
args = TrainingArguments(
    ...
    ## eval plan 기준 Pretrain args(eval_plan 설정값에 따름) START
      ...
      max_steps=200, # gradient_acc_steps=4이기 때문에 /4
    ## eval plan 기준 Pretrain args(eval_plan 설정값에 따름) END
      ...
      # hub_model_id = 'leeseeun/llama2_7b_news_2gb_1024_10epoch', #로컬 output_dir과 동기화할 리포지토리의 이름
)
model.train()
